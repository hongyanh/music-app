...
post '/track/vote' do
  ...
  if VoteRelation.single_vote(session[:user_id], track_id).count == 0
    vote_relation = VoteRelation.new(track_id: track_id, user_id: session[:user_id])
    vote_relation.save
  end
  VoteRelation.where(track_id: track_id).count.to_s
end